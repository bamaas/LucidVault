...
- Apply fixes, run `mise run test` and `mise run lint` to confirm nothing is broken.
- Run /commit. Append the round number to the description: `fix(auth): handle nil token in refresh (round 2)`
