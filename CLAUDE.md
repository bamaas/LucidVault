...
- `MCP_READ_TOOLS` — (optional, default: `false`) Expose the duplicate MCP content-read tools (`read_wiki`, `grep_vault`, `read_note`, `read_raw`, `vault_overview`, `get_soul`). Off by default so filesystem-capable agents read the vault directly (native-first retrieval, see `docs/adr/023-native-first-retrieval-read-tool-gating.md`); enable only for clients without filesystem access. Discovery (`search_wiki`), graph (`related_notes`, `expand_graph`), and write tools are always registered.
- `AGENT_WEB_SEARCH_STRATEGY` — (optional, default: `fallback`) Controls the `## Web Search` section the `AGENTS.md` generator emits, telling an agent how to use its **own** web search relative to the vault: `off` (no web-search guidance anywhere — section and Source-Attribution bullet both omitted), `fallback` (only when the vault lacks coverage), `time-sensitive` (also for latest/current/versions/news/prices/dates questions), `immediately` (web + vault in parallel for any substantive question). All non-`off` modes share the trust-vs-recency, provider-agnostic, and citation rules. LucidVault never provides or names a search provider (see `docs/adr/024-web-search-belongs-to-the-agent.md`). Unknown non-empty values fall back to `fallback` with a logged warning. Parsed in `loadConfig` and threaded into `generateAgentsMD`, mirroring `MCP_READ_TOOLS`.
- `CLAUDE_MD_VAULT_PATH` - (optional, default: falls back to `VAULT_PATH`) The vault's absolute path as seen by whoever *reads* `~/.claude/CLAUDE.md` (the host), not by the pipeline process. Every shipped config sets `VAULT_PATH=/vault`, which does not exist on the host, so this decouples the two (see `docs/adr/028-host-facing-vault-path-for-claudemd.md`). Whitespace-only is treated as unset. Emitted verbatim (surrounding whitespace trimmed), no `~` expansion or other normalisation.
