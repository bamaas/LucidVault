...
| `CLAUDE_MD_VAULT_PATH` | No | - (falls back to `VAULT_PATH`) | Vault path as seen by whoever *reads* `CLAUDE.md` (the host), not by the pipeline. Every shipped config sets `VAULT_PATH=/vault`, which does not exist on the host, so set this to the vault's real host path (e.g. `~/lucid-vault`). |
